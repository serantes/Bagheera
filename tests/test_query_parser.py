...
def test_modified_last_n_days(parser):
    # "MODIFIED LAST 2 DAYS" debería generar un rango desde ayer hasta mañana
...
def test_number_conversion(parser):
    # Verifica que "TWO" se convierta en "2" antes de procesar
    query = parser.parse_date("MODIFIED LAST TWO DAYS")
    # Si funcionó, el resultado será igual que usar el número 2
    assert query == parser.parse_date("MODIFIED LAST 2 DAYS")


def test_ago_expression(parser):
    # "1 YEAR AGO" debería buscar en el año anterior completo
